...
func convertToSwaggerResourceType(path *Path) ResourceType {
	resourceType := ResourceType{
		Display:      path.Name,
		Endpoint:     MustGetEndpointInfoFromURL(path.Operations.Get.Endpoint.TemplateURL, path.Operations.Get.Endpoint.APIVersion),
	...
	if path.Operations.Delete.Permitted {
		resourceType.DeleteEndpoint = MustGetEndpointInfoFromURL(path.Operations.Delete.Endpoint.TemplateURL, path.Operations.Delete.Endpoint.APIVersion)
	}
	if path.Operations.Patch.Permitted {
		resourceType.PatchEndpoint = MustGetEndpointInfoFromURL(path.Operations.Patch.Endpoint.TemplateURL, path.Operations.Patch.Endpoint.APIVersion)
	}
	if path.Operations.Put.Permitted {
		resourceType.PutEndpoint = MustGetEndpointInfoFromURL(path.Operations.Put.Endpoint.TemplateURL, path.Operations.Put.Endpoint.APIVersion)
	}
...
func addConfigPaths(paths []Path, config *Config) ([]Path, error) {
	if config.AdditionalPaths != nil {
		for _, additionalPath := range config.AdditionalPaths {
			...
			endpoint, err := GetEndpointInfoFromURL(path, "")
			if err != nil {
				return []Path{}, err
			}
			newPath := Path{
			...
			if additionalPath.GetPath == "" {
				...
				getEndpoint, err := GetEndpointInfoFromURL(additionalPath.GetPath, "")
				if err != nil {
					return []Path{}, err
				}
...
func GetPathsFromSwagger(doc *loads.Document, config *Config, pathPrefix string) ([]Path, error) {
	...
	for swaggerPath, swaggerPathItem := range swaggerPaths {
		...
		endpoint, err := GetEndpointInfoFromURL(searchPath, swaggerVersion) // logical path
		if err != nil {
			return []Path{}, err
		}
		...
		if getOperation != nil {
			...
			if override.Path == "" || override.RewritePath {
				...
				overriddenEndpoint, err := GetEndpointInfoFromURL(swaggerPath, swaggerVersion)
				if err != nil {
					return []Path{}, err
				}
		...
		if override.DeletePath != "" {
			path.Operations.Delete.Permitted = true
			path.Operations.Delete.Endpoint = MustGetEndpointInfoFromURL(override.DeletePath, path.Endpoint.APIVersion)
		}
		...
		if override.PutPath != "" {
			path.Operations.Put.Permitted = true
			path.Operations.Put.Endpoint = MustGetEndpointInfoFromURL(override.PutPath, path.Endpoint.APIVersion)
		}
...
func countNameSegments(endpoint *EndpointInfo) int {
	count := 0
